...
from utils.utils import Make_vocab, PandasTextDataset, split_dataset
import pandas as pd
...
if __name__=="__main__":
    ...
    padding_size = 30
    train_set = PandasTextDataset(X=x_train, y=y_train, tokenizer=tokenizer, vocab=vocab, padding_size=padding_size)
    val_set = PandasTextDataset(X=x_val, y=y_val, tokenizer=tokenizer, vocab=vocab, padding_size=padding_size)
    test_set = PandasTextDataset(X=x_test, y=y_test, tokenizer=tokenizer, vocab=vocab, padding_size=padding_size)
